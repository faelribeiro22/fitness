...
export default function RunWorkoutScreen() {
  ...
  const { workoutId, dayId } = useLocalSearchParams<{
    workoutId: string;
    dayId: string;
  }>();
  ...
  const currentExercise = currentItem
    ? exerciseMap.get(currentItem.exerciseId)
    : undefined;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(
      () => setRestSeconds(value => Math.max(0, (value ?? 1) - 1)),
      1000,
    );
  ...
  async function completeSet() {
    ...
    if (!weight.trim() || !reps.trim()) {
      Alert.alert(
        'Preencha a série',
        'Informe peso e repetições antes de concluir.',
      );
      return;
    }
    ...
    return (
      <SafeAreaView style={s.safe}>
        <View style={s.complete}>
          ...
          <Text style={s.completeTitle}>Treino concluído!</Text>
          <Text style={s.completeCopy}>
            Você treinou por {formatTime(elapsed)}. Seu histórico foi salvo
            localmente.
          </Text>
          <Pressable
            style={s.primary}
          ...
          >
            <Text style={s.primaryText}>Voltar aos treinos</Text>
          </Pressable>
        </View>
      </SafeAreaView>
    );
  if (!running || !currentItem || !currentExercise)
    return (
      <SafeAreaView style={s.safe}>
        <View style={s.loading}>
          <Text style={s.loadingText}>Preparando treino...</Text>
        </View>
      </SafeAreaView>
    );
      ...
      <View style={s.page}>
        <View style={s.header}>
          <Pressable onPress={() => router.back()}>
            <Text style={s.back}>‹ Encerrar</Text>
          </Pressable>
          <View style={s.timerArea}>
            ...
            <Pressable
              ...
              style={s.pauseButton}
            >
              <Text style={s.pauseText}>{paused ? 'Retomar' : 'Pausar'}</Text>
            </Pressable>
          </View>
        </View>
        <ScrollView
        ...
        >
          <Text style={s.eyebrow}>
            {running.day.weekday.toUpperCase()} · EXERCÍCIO {exerciseIndex + 1}{' '}
            DE {running.day.exercises.length}
          </Text>
          <Text style={s.exerciseName}>{currentExercise.name}</Text>
          <Text style={s.exerciseInfo}>
            {currentExercise.muscleGroup} · {currentExercise.equipment}
          </Text>
          {restSeconds !== null ? (
            <View style={s.restCard}>
              <Text style={s.restLabel}>DESCANSO AUTOMÁTICO</Text>
              <Text style={s.restTime}>{formatTime(restSeconds)}</Text>
              <Text style={s.restCopy}>Prepare-se para a próxima série.</Text>
              <Pressable
                onPress={() => {
                  setRestSeconds(null);
                  advance();
              ...
              >
                <Text style={s.skip}>Pular descanso</Text>
              </Pressable>
            </View>
          ) : (
            <>
              <View style={s.setCard}>
                ...
                <Text style={s.fieldLabel}>OBSERVAÇÃO DA SÉRIE</Text>
                <TextInput
                  value={seriesNote}
                  onChangeText={setSeriesNote}
                  multiline
                  placeholder="Ex.: última repetição difícil"
                  placeholderTextColor="#999F97"
                  style={s.setNotes}
                />
              </View>
              <Pressable style={s.primary} onPress={() => void completeSet()}>
                <Text style={s.primaryText}>✓ Concluir série</Text>
              </Pressable>
              <Pressable style={s.next} onPress={advance}>
                <Text style={s.nextText}>Próximo exercício ›</Text>
              </Pressable>
            </>
          )}
          ...
          <TextInput
            ...
            placeholder="Como você se sentiu? Algum ajuste para a próxima vez?"
            placeholderTextColor="#999F97"
            style={s.notes}
          />
          <Text style={s.section}>HISTÓRICO DO EXERCÍCIO</Text>
          {history.length ? (
            history.map((item, index) => (
              <View key={`${item.completedAt}-${index}`} style={s.history}>
                <Text style={s.historyDate}>
                  {new Date(item.completedAt).toLocaleDateString('pt-BR')}
                </Text>
                <View>
                  <Text style={s.historyValue}>
                    Série {item.setNumber} · {item.weight} kg ×{' '}
                    {item.repetitions}
...
const s = StyleSheet.create({
  ...
  back: { color: '#5C626B', fontSize: 14, fontWeight: '800', zIndex: 1 },
  timerArea: {
    alignItems: 'center',
    flexDirection: 'row',
    gap: 10,
  ...
  timer: { color: '#273022', fontSize: 15, fontWeight: '900' },
  pauseButton: {
  ...
  pauseText: { color: '#597E31', fontSize: 11, fontWeight: '900' },
  ...
  exerciseInfo: { color: '#717781', fontSize: 15, marginTop: 6 },
  setCard: {
  ...
  fields: { flexDirection: 'row', gap: 12, marginTop: 22 },
  field: { flex: 1 },
  rpeField: { flex: 0.55 },
  fieldLabel: {
    color: '#777D85',
  ...
  unit: { color: '#707680', fontSize: 16, fontWeight: '800', marginLeft: 6 },
  setNotes: {
  ...
  next: { alignItems: 'center', paddingVertical: 17 },
  nextText: { color: '#5C9028', fontSize: 14, fontWeight: '900' },
  restCard: {
    alignItems: 'center',
  ...
  restTime: { color: '#30451A', fontSize: 54, fontWeight: '900', marginTop: 8 },
  restCopy: { color: '#65735A', fontSize: 14, marginTop: 5 },
  skip: { color: '#5B9027', fontSize: 14, fontWeight: '900', marginTop: 22 },
  section: {
    color: '#747A83',
    fontSize: 10,
    fontWeight: '900',
  ...
  historyDate: { color: '#747A83', fontSize: 12 },
  historyValue: {
    color: '#30343A',
    fontSize: 13,
    fontWeight: '800',
  ...
  emptyHistory: { color: '#858A91', fontSize: 13, marginTop: 11 },
  loading: { alignItems: 'center', flex: 1, justifyContent: 'center' },
  loadingText: { color: '#707680', fontSize: 15 },
  complete: {
    alignItems: 'center',
    flex: 1,
    justifyContent: 'center',
