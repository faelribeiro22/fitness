...
import { getUserProfile, saveUserProfile } from '@/db/workouts';
...
const goals: { name: Goal; description: string; icon: string }[] = [
  ...
  {
    name: 'Resistência',
    description: 'Melhorar seu condicionamento',
    icon: '∞',
  },
];
...
export default function HomeScreen() {
  ...
  useEffect(() => {
    let mounted = true;
    void getUserProfile(db)
      .then(profile => {
        if (!mounted) return;
        if (profile) router.replace('/exercises');
  ...
  const canContinue = useMemo(() => {
    ...
    if (step === 1)
      return Number(weight.replace(',', '.')) > 0 && Number(height) > 0;
  ...
  return (
    <>
      ...
      <Text style={styles.description}>
        Vamos personalizar sua experiência a partir daqui.
      </Text>
...
function MeasurementsStep({
  ...
  return (
    <>
      ...
      <Text style={styles.description}>
        Esses dados nos ajudam a montar um plano adequado para você.
      </Text>
...
function ProfileStep({
  ...
  return (
    <>
      ...
      <Text style={styles.description}>
        As informações ficam privadas e ajudam a ajustar suas recomendações.
      </Text>
      <View style={styles.fieldGroup}>
...
function GoalStep({
  ...
  return (
    <>
      ...
      <Text style={styles.description}>
        Escolha seu foco principal. Você poderá alterá-lo quando quiser.
      </Text>
      <View style={styles.goalList}>
        {goals.map(item => (
          <Pressable
            key={item.name}
            onPress={() => onGoalChange(item.name)}
            style={[
              styles.goalCard,
              goal === item.name && styles.goalCardSelected,
            ]}
          >
            <View
              style={[
                styles.goalIcon,
                goal === item.name && styles.goalIconSelected,
              ]}
            >
              <Text
                style={[
                  styles.goalIconText,
                  goal === item.name && styles.goalIconTextSelected,
                ]}
              >
                {item.icon}
              </Text>
            ...
            <View
              style={[styles.radio, goal === item.name && styles.radioSelected]}
            >
              {goal === item.name && <View style={styles.radioDot} />}
            </View>
          </Pressable>
        ))}
      </View>
...
const styles = StyleSheet.create({
  ...
  radioSelected: { borderColor: '#75AC36' },
  radioDot: {
    backgroundColor: '#75AC36',
    borderRadius: 99,
    height: 10,
    width: 10,
  },
  ...
  loadingText: { color: '#686D78', fontSize: 15, fontWeight: '700' },
});
