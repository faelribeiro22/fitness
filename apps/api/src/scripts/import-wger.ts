  ...
  while (url) {
    ...
    for (const exercise of page.results ?? []) {
      ...
      await client.exercise.upsert({
        where: {
          source_sourceId: { source: 'wger', sourceId: String(exercise.id) },
        },
        create: {
          source: 'wger',
          sourceId: String(exercise.id),
          name: exercise.name,
          muscleGroup: exercise.category?.name ?? 'Outros',
          secondaryMuscles:
          ...
          instructions: exercise.description
            ? [exercise.description.replace(/<[^>]+>/g, '').trim()]
            : [],
          image: exercise.images?.[0]?.image ?? null,
          video: exercise.videos?.[0]?.video ?? null,
        },
        update: {
          name: exercise.name,
          muscleGroup: exercise.category?.name ?? 'Outros',
